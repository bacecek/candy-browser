...
| Blank-tab editor | `ui/NewTabPage.kt`, `ui/ExpandedAddressBar.kt` | Keep the localized Favorites heading and regular-tab favorites visible and actionable while address input is focused; hide the complete section in private mode or when no favorites exist. Keep the private-mode toggle immediately after the address input, including while the focused editor uses the full available width. |
...
| Automatic right parking | `AddressBarAutoDockRules`, `TextInputOcclusionScript`, Android engine session ports | After a successful main-frame load or same-document URL navigation, the selected regular tab gets one bounded probe after layout settles. The current address-bar viewport rectangle must overlap a visible text-like `input`, `textarea`, or `contenteditable` editor while the top-level document has no remaining downward scroll. GeckoView routes the fixed probe through Candy's internal extension; System WebView evaluates the same fixed predicate. No DOM mutation or scroll observer is installed. Tab, engine session, navigation generation, final URL, chrome geometry and docking availability must remain current before applying the result. Private tabs are excluded because parking persists globally. A successful result uses normal right parking and remains parked until the user restores the address bar. |
| Link Peek | Shared [`LinkPeekActionLayout.kt`](../../shared/src/commonMain/kotlin/dev/sk2andy/materialbrowser/browser/LinkPeekActionLayout.kt); Android [`LinkPeek.kt`](../../app/src/main/java/dev/sk2andy/materialbrowser/ui/LinkPeek.kt), `WebContentActionState`, `BrowserContentTargetRules` | Shared policy owns the stable action catalog, wire values, normalized configurable layout and runtime slot projection. Android retains measured action-bar geometry, drag/editor UI, persistence and Gecko-backed execution as adapters. The preview has exactly four positions. Three configurable positions accept unique actions or remain empty; dragging an action back to the palette preserves its empty position. Only empty configurable positions are drop targets, so occupied actions and the pulsing background-tab `+` never show a drop marker. The `+` is fixed in the third, penultimate position. The catalog contains Reader offline, private-open, copy, share, favorite, snooze, and foreground-tab open. Every quick action uses the current committed HTTP(S) URL after redirects; explicit downloads retain the original target URL and source-tab session headers. Gecko `ContentDelegate.onContextMenu` crosses the normalized content-target boundary without exposing `GeckoSession` to action state. Switching tabs or starting a new source navigation invalidates the target, and only `+` owns commit motion. iOS must add native preview gestures and outcome executors before consuming this shared layout; no platform action is synthesized. |
...
| Long-press page content | Shared [`LinkLongPress.kt`](../../shared/src/commonMain/kotlin/dev/sk2andy/materialbrowser/browser/LinkLongPress.kt), Android [`BrowserContentActions.kt`](../../app/src/main/java/dev/sk2andy/materialbrowser/browser/actions/BrowserContentActions.kt) and [`BrowserContentTarget.kt`](../../app/src/main/java/dev/sk2andy/materialbrowser/browser/actions/BrowserContentTarget.kt) | Shared policy owns stable action IDs and maps link/private capability to a semantic outcome. Android retains the Tabs & gestures editor, persistence, normalized content target and engine-scoped execution as platform adapters. The preference selects Link Peek (default), copy, share, download, or an explicit regular/private foreground/background tab destination. Every dispatched choice emits one short haptic. Successful background creation uses the shared subtle address-bar pulse while the source tab stays selected. Existing stored regular-background and private-foreground choices retain their stable IDs; unsupported private opens fall back to Link Peek. Direct link downloads reuse the existing engine-scoped context-download path; Gecko retains its source referrer and private session context. Gecko image-link context elements retain both link and image actions, while image-only targets keep the content sheet and video/audio sources are not mislabeled as image downloads. Renderable `.txt` and `.json` links remain viewable on normal taps and can be sent explicitly to the configured download manager from Link Peek. iOS still needs a native content-gesture adapter plus outcome executors before exposing this preference; no editor or no-op action is synthesized. |
